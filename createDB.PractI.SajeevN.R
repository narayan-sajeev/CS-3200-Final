# createDB.PractI.SajeevN.R
# Narayan Sajeev
# Summer 2 2025

# Load required library
library(RMySQL)

# Define settings
db_host_aiven <- "mysql-cs3200-final-northeastern-526e.l.aivencloud.com"
db_port_aiven <- 28849
db_name_aiven <- "defaultdb"
db_user_aiven <- "avnadmin"
db_pwd_aiven <- "AVNS_6TJXHpMjE7TJa78fR1w"

# Embedded SSL certificate
db_cert <- "
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
"

# Connect to MySQL server
con <-  dbConnect(RMySQL::MySQL(), 
                         user = db_user_aiven, 
                         password = db_pwd_aiven,
                         dbname = db_name_aiven, 
                         host = db_host_aiven, 
                         port = db_port_aiven,
                         sslmode = "require",
                         sslcert = db_cert)

# Create airline table
dbExecute(con, "CREATE TABLE IF NOT EXISTS Airline (
          airlineCode CHAR(2) PRIMARY KEY
          )")

# Create airport table
dbExecute(con, "CREATE TABLE IF NOT EXISTS Airport (
          depAirport CHAR(3) PRIMARY KEY
          )")

# Create aircraft table
dbExecute(con, "CREATE TABLE IF NOT EXISTS Aircraft (
          aircraftName VARCHAR(15) PRIMARY KEY
          )")

# Create incidentType table
dbExecute(con, "CREATE TABLE IF NOT EXISTS IncidentType (
          incidentName VARCHAR(10) PRIMARY KEY
          )")

# Create severity table
dbExecute(con, "CREATE TABLE IF NOT EXISTS Severity (
          severityType VARCHAR(8) PRIMARY KEY
          CHECK (severityType in ('minor', 'critical', 'major'))
          )")

# Create reporter table
dbExecute(con, "CREATE TABLE IF NOT EXISTS Reporter (
          reporterName VARCHAR(13) PRIMARY KEY
          CHECK (reporterName in ('crew', 'first officer', 'mechanic', 'ground staff', 'company', 'pilot'))
          )")

# Create airline table
dbExecute(con, "CREATE TABLE IF NOT EXISTS Flight (
          airlineCode CHAR(2) NOT NULL,
          flightNumber INTEGER NOT NULL
          CHECK (flightNumber > 0),
          date DATE NOT NULL,
          depAirport CHAR(3) NOT NULL,
          aircraftName VARCHAR(15) NOT NULL,
          PRIMARY KEY (airlineCode, flightNumber, date),
          FOREIGN KEY (airlineCode) REFERENCES Airline(airlineCode),
          FOREIGN KEY (depAirport) REFERENCES Airport(depAirport),
          FOREIGN KEY (aircraftName) REFERENCES Aircraft(aircraftName)
          )")

# Create incident table
dbExecute(con, "CREATE TABLE IF NOT EXISTS Incident (
          iid VARCHAR(6) PRIMARY KEY,
          airlineCode CHAR(2) NOT NULL,
          flightNumber INTEGER NOT NULL,
          date DATE NOT NULL,
          incidentName VARCHAR(10) NOT NULL,
          severityType VARCHAR(8) NOT NULL,
          delay INTEGER NOT NULL
          DEFAULT 0
          CHECK (delay >= 0),
          numInjuries INTEGER NOT NULL
          DEFAULT 0
          CHECK (numInjuries >= 0 AND numInjuries < 3),
          reporterName VARCHAR(13) NOT NULL,
          FOREIGN KEY (airlineCode, flightNumber, date) REFERENCES Flight(airlineCode, flightNumber, date),
          FOREIGN KEY (incidentName) REFERENCES IncidentType(incidentName),
          FOREIGN KEY (severityType) REFERENCES Severity(severityType),
          FOREIGN KEY (reporterName) REFERENCES Reporter(reporterName)
          )")

# Disconnect from database
dbDisconnect(con)