...
dbGetQuery(con, "SELECT MONTHNAME(date) as Month,
           ...
           ORDER BY MONTH(date), IncidentName") %>%
          kable(format = "latex", caption="Incident Type and Frequency by Month", longtable = TRUE)
```
...
dbGetQuery(con, "SELECT * FROM AirlineView") %>%
          kable(format = "latex", caption="Incident Count and Average Delay by Airline", longtable = TRUE)
})
```
