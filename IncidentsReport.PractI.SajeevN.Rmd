...
dbGetQuery(con, "SELECT MONTHNAME(date) as Month,
           ...
           COUNT(*) as Count
           FROM Incident
           GROUP BY Month, MONTH(date), IncidentName
           ORDER BY MONTH(date)") %>%
...
```{sql dropView, connection=con, echo=FALSE}
DROP VIEW IF EXISTS AirlineView
```

```{sql createAirlineView, connection=con, echo=FALSE}
CREATE VIEW AirlineView AS
